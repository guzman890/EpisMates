...
var BootboxWrapper = {
     ...
     */
    showModalAlert: function showModalAlert(titleText, messageText, okButtonText, color) {
        bootbox.dialog({
            title: titleText,
            message: messageText,
            buttons: {
     ...
     */
    showModalConfirmation: function showModalConfirmation(titleText, messageText, okCallback, cancelCallback, okButtonText, cancelButtonText, color) {
        bootbox.dialog({
            title: titleText,
            message: messageText,
            buttons: {
     ...
     *                  color (defaults to StatusType.INFO)
     */
    showModalConfirmationWithCancel: function showModalConfirmationWithCancel(titleText, messageText, yesButtonCallback, noButtonCallback, cancelButtonCallback, yesButtonText, noButtonText, cancelButtonText, color) {
        bootbox.dialog({
            title: titleText,
            message: messageText,
            buttons: {
...
function sortTable(oneOfTableCell, colIdx, comp, ascending, row) {
    ...
    var textToCompare = void 0;
    for (var i = row; i < $RowList.length; i += 1) {
        ...
        textToCompare = shouldConsiderToolTipYear ? $.trim($($RowList[i].cells[colIdx - 1]).find('span').attr('data-original-title')) : $.trim($($RowList[i].cells[colIdx - 1]).text());
    ...
    store.sort(function (x, y) {
        var compareResult = ascending ? comparator(x[0].toUpperCase(), y[0].toUpperCase()) : comparator(y[0].toUpperCase(), x[0].toUpperCase());
        if (compareResult === 0) {
            return x[2] - y[2];
        }
