...
function sendEmailToNewEmailOption(event, newStudentEmail) {
    ...
    var messageText = 'Do you want to resend past session links of this course to the new email ' + newStudentEmail + '?';
    var yesCallback = function yesCallback() {
    ...
    BootboxWrapper.showModalConfirmationWithCancel('Resend past links to the new email?', messageText, yesCallback, noCallback, null, 'Yes, save changes and resend links', 'No, just save the changes', 'Cancel', StatusType.PRIMARY);
}
...
function readyInstructorStudentEditPage() {
    $(INSTRUCTOR_STUDENT_EDIT_FORM).on('submit', function (event) {
        ...
        if ($(INSTRUCTOR_STUDENT_EDIT_FORM).attr('editStatus') === 'mustDeleteResponses') {
            ...
            var messageText = 'Editing these fields will result in some existing responses from this student ' + 'to be deleted. You may download the data before you make the changes. Are ' + 'you sure you want to continue?';
