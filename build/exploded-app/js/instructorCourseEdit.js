...
var instructorPrivilegeValues = ['canmodifycourse', 'canmodifyinstructor', 'canmodifysession', 'canmodifystudent', 'canviewstudentinsection', 'cangivecommentinsection', 'canviewcommentinsection', 'canmodifycommentinsection', 'cansubmitsessioninsection', 'canviewsessioninsection', 'canmodifysessioncommentinsection'];
...
function setAddSectionLevelLink(instrNum) {
    ...
    for (var idx = 0; idx < allSectionSelects.length; idx += 1) {
...
function enableEditInstructor(event) {
    ...
    for (var i = 1; i <= totalInstructors; i += 1) {
        if (i === instructorNum) {
...
function checkPrivilegesOfCoownerForInstructor(instrNum) {
    ...
    for (var i = 0; i < instructorPrivilegeValues.length; i += 1) {
...
function checkPrivilegesOfCustomForInstructor(instrNum) {
    ...
    if (instrNum <= numOfInstr && instrNum <= instructorCourseEditInstructorAccessLevelWhenLoadingPage.length) {
        ...
        for (var j = 0; j < instructorPrivilegeValues.length; j += 1) {
...
function bindDeleteInstructorLink() {
    $('[id^="instrDeleteLink"]').on('click', function (event) {
        ...
        var messageText = $clickedLink.data('isDeleteSelf') ? 'Are you sure you want to delete your instructor role from the course <strong>' + $clickedLink.data('courseId') + '</strong>? ' + 'You will not be able to access the course anymore.' : 'Are you sure you want to delete the instructor <strong>' + $clickedLink.data('instructorName') + '</strong> from the course <strong>' + $clickedLink.data('courseId') + '</strong>? ' + 'He/she will not be able to access the course anymore.';
        var okCallback = function okCallback() {
            window.location = $clickedLink.attr('href');
        };

        BootboxWrapper.showModalConfirmation('Confirm deleting instructor', messageText, okCallback, null, BootboxWrapper.DEFAULT_OK_TEXT, BootboxWrapper.DEFAULT_CANCEL_TEXT, StatusType.DANGER);
    });
}
...
function bindRemindInstructorLink() {
    $('[id^="instrRemindLink"]').on('click', function (event) {
        ...
        var messageText = 'Do you wish to re-send the invitation email to instructor <strong>' + $clickedLink.data('instructorName') + '</strong> from course <strong>' + $clickedLink.data('courseId') + '</strong>';
        var okCallback = function okCallback() {
            window.location = $clickedLink.attr('href');
        };

        BootboxWrapper.showModalConfirmation('Confirm re-sending invitation email', messageText, okCallback, null, BootboxWrapper.DEFAULT_OK_TEXT, BootboxWrapper.DEFAULT_CANCEL_TEXT, StatusType.INFO);
    });
}
...
function bindChangingRole(index) {
    $('input[id^=\'instructorroleforinstructor' + index + '\']').change(function () {
        ...
        var instrNum = parseInt(idAttr.substring(27), 10);
...
$(document).ready(function () {
    ...
    var courseTimeZone = $('#course-time-zone').val();
