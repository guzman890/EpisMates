...
$(document).ready(function () {
    ...
    $.each($radioButtons, function () {
        $(this).click(function () {
            ...
            for (var i = 0; i < paramValues.length; i += 1) {
    ...
    $('body').on('click', '.course-archive-for-test', function (event) {
        ...
        var messageText = 'Are you sure you want to archive ' + $clickedLink.data('courseId') + '? ' + 'This action can be reverted by going to the "courses" tab and unarchiving the desired course(s).';
        var okCallback = function okCallback() {
            window.location = $clickedLink.attr('href');
        };

        BootboxWrapper.showModalConfirmation('Confirm archiving course', messageText, okCallback, null, BootboxWrapper.DEFAULT_OK_TEXT, BootboxWrapper.DEFAULT_CANCEL_TEXT, StatusType.INFO);
    });
