...
$(document).ready(function () {
    ...
    $('form.form_comment').submit(function (e) {
        if ($(e.currentTarget).find('[id^=commentedittype]').val() !== 'delete') {
            var commentTextInput = $(e.currentTarget).find('.mce-content-body');
    ...
    function hideCommentAndItsPanel(comment) {
        ...
        if (commentToHide.hasClass('student-record-comments')) {
            ...
            var allStudentCommentsAreHidden = studentCommentPanel.find('div[class*="giver_display-by"][style*="display: none"]').length === studentCommentPanel.find('div[class*="giver_display-by"]').length;
            if (allStudentCommentsAreHidden) {
                studentCommentPanelBody.hide();
            }
        } else {
            // to hide feedback question + feedback session panel
            var allCommentsForFeedbackResponseAreHidden = commentToHide.parent().find('li[style*="display: none"]').length === commentToHide.parent().find('li').length;
            if (allCommentsForFeedbackResponseAreHidden) {
                ...
                var feedbackQuestion = commentListRegionForFeedbackResponse.closest('.feedback-question-panel');
                var allFeedbackResponsesForFeedbackQuestionAreHidden = feedbackQuestion.find('tr[style*="display: none"]').length === feedbackQuestion.find('tr[class*="table-row"]').length;
                if (allFeedbackResponsesForFeedbackQuestionAreHidden) {
                    ...
                    var feedbackSessionPanelBody = feedbackQuestion.parent();
                    var allFeedbackQuestionsForFeedbackSessionAreHidden = feedbackSessionPanel.find('div[class="panel panel-info"][style*="display: none"]').length === feedbackSessionPanel.find('div[class="panel panel-info"]').length;
    ...
    $('input[type=checkbox]').click(function (e) {
        ...
        if ((target.prop('class').includes('giverCheckbox') || target.prop('class').includes('recipientCheckbox')) && target.prop('checked')) {
